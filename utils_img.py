...
def get_image_paths(dataroot, natsort=False):
    paths = None  # return None if dataroot is None
    if isinstance(dataroot, str):
        # paths = sorted(_get_paths_from_images(dataroot, natsort=natsort))
        paths = _get_paths_from_images(dataroot, natsort=natsort)
    elif isinstance(dataroot, list):
        paths = []
        for i in dataroot:
            paths += sorted(_get_paths_from_images(i, natsort=natsort))
    return paths


def _get_paths_from_images(path, natsort=False):
    ...
    for dirpath, _, fnames in os.walk(path):
        # print(sort_fn(fnames))
        for fname in sort_fn(fnames):
