...
class GradientSReconstructor():
    """Instantiate a reconstruction algorithm."""

    def __init__(self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1, loss_fn=torch.nn.CrossEntropyLoss(reduction='mean'), sr_scale=1):
        """Initialize with algorithm setup."""
        self.config = _validate_config(config)
        self.model = model
        self.setup = dict(device=next(model.parameters()).device, dtype=next(model.parameters()).dtype)
        ...
        self.upsampler = torch.nn.Upsample(scale_factor=self.sr_scale, mode='bicubic')
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None, init_data=None, save_verbose=False, save_dir=None):
        ...
        stats = defaultdict(list)
        c, h,w = img_shape
        sr_h, sr_w = h//self.sr_scale, w//self.sr_scale
        img_shape = (c, sr_h, sr_w)
        x = self._init_images(img_shape)
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            x_sr = self.upsampler(x_trial)
            loss = self.loss_fn(self.model(x_sr), label)
            param_list = [param for param in self.model.parameters() if param.requires_grad]
            gradient = torch.autograd.grad(loss, param_list, create_graph=True)
            rec_loss = reconstruction_costs([gradient], input_gradient,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
