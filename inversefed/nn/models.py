...
def construct_model(model, num_classes=10, seed=None, num_channels=3, modelkey=None):
    ...
    if model in ['ConvNet', 'ConvNet64']:
    ...
    elif model == 'ConvNet8_embed':
        model = ConvNet_embed(width=8, num_channels=num_channels, num_classes=num_classes)
        ...
        model = ResNet(torchvision.models.resnet.BasicBlock, [4, 4, 4], num_classes=num_classes, num_channels=num_channels, base_width=16 * 10)
    elif model == 'ResNet32':
        model = ResNet(torchvision.models.resnet.BasicBlock, [5, 5, 5], num_classes=num_classes, num_channels=num_channels, base_width=16)
    elif model == 'ResNet32-10':
        model = ResNet(torchvision.models.resnet.BasicBlock, [5, 5, 5], num_classes=num_classes, num_channels=num_channels, base_width=16 * 10)
    elif model == 'ResNet44':
        model = ResNet(torchvision.models.resnet.BasicBlock, [7, 7, 7], num_classes=num_classes, num_channels=num_channels, base_width=16)
    elif model == 'ResNet56':
        model = ResNet(torchvision.models.resnet.BasicBlock, [9, 9, 9], num_classes=num_classes, num_channels=num_channels, base_width=16)
        ...
        model = _DenseNet(_Bottleneck, [6, 6, 6, 0], growth_rate=12, num_channels=num_channels, num_classes=num_classes)
...
class ConvNet_embed(torch.nn.Module):
    """ConvNetBN."""
    ...
    def __init__(self, width=32, num_classes=10, num_channels=3):
        ...
        self.classifier = torch.nn.Linear(36 * width, num_classes)
    def embed(self, input):
        out = self.features(input)
        out = self.flatten(out)
        return out # 1152 for 28*28*3
    def forward(self, input):
        out = self.features(input)
