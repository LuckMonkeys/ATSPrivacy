...
def reconstruct(idx, model, loss_fn, trainloader, validloader, mean_std, shape, label_key):
    ...
    torch.save(input_gradient, f'gradient_{shape[1]}.pt')
    exit(0)
...
def create_checkpoint_dir():
    if opt.fix_ckpt:
        ...
        name = '_'.join(name_and_size[1:-1])
        size = name_and_size[-1]
        save_dir = f'checkpoints/scale_{name}/data_{name}_arch_{opt.arch}_mode_{opt.mode}_auglist_{opt.aug_list}_rlabel_{opt.rlabel}_{224}'
        print('load check point from ', save_dir)
        return save_dir
